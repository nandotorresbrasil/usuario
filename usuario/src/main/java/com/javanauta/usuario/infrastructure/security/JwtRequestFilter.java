...
import com.fasterxml.jackson.datatype.jsr310.JavaTimeModule; // Evita erro ao serializar LocalDateTime
import com.javanauta.usuario.infrastructure.exceptions.dto.ErrorResponseDTO;
import io.jsonwebtoken.ExpiredJwtException; // Importação adicionada para capturar o token expirado
...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        try {
            ...
            response.setContentType("application/json"); // Define que o retorno do erro é um JSON
    ...
    // Converte o erro para JSON de forma segura sem estourar assinatura de método do Spring
    private String buildError(int status, String mensagem, String path) {
        try {
            ...
            objectMapper.registerModule(new JavaTimeModule()); // Registra suporte ao LocalDateTime
            return objectMapper.writeValueAsString(errorResponseDTO);
        } catch (JsonProcessingException e) {
            return "{\"message\": \"" + mensagem + "\", \"status\": " + status + "}";
        }
    }
}